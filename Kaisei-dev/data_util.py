...
def restore_rectangle_rbox(origin, geometry):
    """
    :param origin: (x, y) under input size
    :param geometry: (y, x) convention, n * 5, n is the num of points in origin
    :return:
    """
    d = geometry[:, :4]
    angle = geometry[:, 4]
...
def yet_another_rbox_restore(origin, geometry):
    """
    :param origin: (x, y) under input size
    :param geometry: n * 5, n is the num of points in origin
    :return:
    """
    # dist to t, r, b, l
    d = geometry[:, :4]
    ...
    for ind, p in enumerate(origin):
        ...
        if not cover_flag:
            ...
            h = d[ind][0] + d[ind][2]
            w = d[ind][1] + d[ind][3]
            dx = (tangle * h - w) / (np.square(tangle) - 1)
            dy = tangle * dx
            p0 = [x + dx, y]
            p1 = [x, y + dy]
            p2 = [x + w - dx, y + h]
            p3 = [x + w, y + h - dy]
            box = [p0, p1, p2, p3]
            restored.append(Polygon(box))
            restored_boxes.append(box)
            cover_flag = False
    return np.array(restored_boxes)
    # for angle > 0
    # origin_0 = origin[angle >= 0]
    # d_0 = d[angle >= 0]
    # angle_0 = angle[angle >= 0]
